...
def generate_text_datasets(graph_path, entity_path, chunk_path,
    ...
    Dựa trên liên kết entity–chunk trong đồ thị, KHÔNG dùng fact.
    """
    ...
    # 4️⃣ Map entity → chunks (từ graph)
    entity_to_chunks = {}
    for v in g.vs:
    ...
    pairs = []
    for ent_hash, chunk_ids in tqdm(entity_to_chunks.items(), desc="Generating pairs"):
        phrase_id = node_name_to_idx[ent_hash]
        for ch_idx in chunk_ids:
            passage_id = ch_idx
